...
declare namespace getRegistrationFormAnswerSharedFile {
  type Props = Readonly<{
    props: {
      answerId: string
      sharingId: string
...
const getRegistrationFormAnswerSharedFile = async ({
  props: { answerId, sharingId },
  idToken,
}: getRegistrationFormAnswerSharedFile.Props): Promise<
  | { blob: Blob; filename?: string }
  | { errorCode: "fileNotFound" | "formAnswerNotFound"; error?: any }
> => {
  try {
    const res = await client({ idToken }).get(
      "registration-form-answer/file-sharing/get-file",
      {
        searchParams: {
          answer_id: answerId,
...
export { getRegistrationFormAnswerSharedFile }
